...
-- 기간별 보기 월별 자유 메모 (year_month → 텍스트)
...
-- ------------------------------------------------------------
-- 14. 홈 알림바 설정 (멘트 탭 문구 등, 기기/브라우저 동기화)
-- ------------------------------------------------------------
...
comment on table alert_bar_settings is '알림바: system_overrides(멘트·수면·일정 등), custom_alerts(사용자 추가 문구)';
insert into alert_bar_settings (id, system_overrides, custom_alerts) values ('default', '{}', '[]') on conflict (id) do nothing;
