...
### 일기 비밀글 + 일기 초안 (기기/브라우저 동기화, 새로고침 유지)

일기 **비밀글(secret)** 컬럼과 **저장 전 초안**을 PC·모바일·모든 브라우저에서 맞추고, 새로고침해도 초안이 사라지지 않게 하려면 **SQL Editor → New query**에서 아래 파일을 실행하세요.

- **`migration-journal-secret-and-drafts.sql`**
  - `journal_entries`에 `secret` 컬럼 추가 (비밀글)
  - `journal_drafts` 테이블 생성 (날짜별 초안: content, important, secret)

실행 후에는 일기 쓰다가 저장하지 않아도 다른 기기에서 이어서 볼 수 있고, 새로고침해도 초안이 유지됩니다.

---

### 포스트잇 메모 카테고리 (유튜브·라이프·기타 등)

메모 페이지에서 **카테고리별로 포스트잇을 나눠 보관**하려면 **SQL Editor**에서 **`migration-memo-categories.sql`** 을 실행하세요. (`memo_categories` 테이블 + `memos.category_id` 컬럼)

메모 **겹침 순서**(선택·새 메모가 위로)를 기기 간에 맞추려면 **`migration-memos-stack-order.sql`** (`memos.stack_order`)도 실행하세요. 컬럼 없으면 로컬만 동작하고 Supabase upsert 시 경고가 날 수 있습니다.
...
| **journal_entries** | 일기 (날짜당 1건) | `date` unique, `secret`(비밀글) |
| **journal_drafts** | 일기 초안 (저장 전 글) | 기기·브라우저·새로고침 연동용, migration-journal-secret-and-drafts.sql |
